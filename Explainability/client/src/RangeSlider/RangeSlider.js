...
// Modified from https://github.com/whoisandy/react-rangeslider
...
class Slider extends Component {
  ...
  coordinates = pos => {
    ...
    const { orientation, startZero } = this.props
    const value = this.getValueFromPosition(pos)
    const position = this.getPositionFromValue(value)
    const zeroPos = startZero ? this.getPositionFromValue(0) + grab : 0
    const handlePos = orientation === 'horizontal' ? position + grab : position
    const fillPos = startZero
      ? Math.abs(handlePos - zeroPos)
      : (orientation === 'horizontal'
        ? handlePos
        : limit - handlePos)
    const fillStart = !startZero
      ? 0
      : (value > 0
        ? zeroPos
        : handlePos)
